"""
Script to analyze the structure of the APCs (article processing charges) 
that journals indexed by DOAJ charge. 
...
def apc_in_eur(data): 
    ...
    print("\nSummary statistics, nonzero journals")
    nonzerodata = data[data["APC-EUR"] > 0]
    print("min:", np.min(nonzerodata["APC-EUR"]))
    print("median:", np.median(nonzerodata["APC-EUR"]))
    print("mean:", np.mean(nonzerodata["APC-EUR"]))
    print("max:", np.max(nonzerodata["APC-EUR"]))
    print("stdev:", np.std(nonzerodata["APC-EUR"]))
...
def make_histogram0(data): 
    ...
    plt.savefig(join(workdir, "..", "figures", "APC-histogram_all.png"), dpi=300)
    plt.close()
...
def make_histogram1(data): 
    ...
    plt.savefig(join(workdir, "..", "figures", "APC-histogram_nonzero.png"), dpi=300)
    plt.close()
...
def make_histogram3(data): 
    ...
    plt.savefig(join(workdir, "..", "figures", "APC-histogram_low.png"), dpi=300)
    plt.close()
...
def make_histogram4(data): 
    ...
    plt.savefig(join(workdir, "..", "figures", "APC-histogram_high.png"), dpi=300)
    plt.close()
